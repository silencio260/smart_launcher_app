...
/// shortcut) so the two privacy mini-apps feel identical.
///
/// Calls [onUnlocked] once the user is in; [onCancel] when they back out.
class AppLockLockScreen extends StatefulWidget {
  final AppLockSecurityRepository security;
  ...
  @override
  State<AppLockLockScreen> createState() => _AppLockLockScreenState();
}

class _AppLockLockScreenState extends State<AppLockLockScreen> {
  ...
  AppLockSecurityRepository get _sec => widget.security;
  ...
  void _handleSetupCode(String code) {
    ...
    if (code != _first) {
      setState(() {
        _step = 0;
        _first = null;
        _error = _type == AppLockSecurityRepository.typePin
    ...
    final result = await showDialog<bool>(
      ...
      builder: (context) => Theme(
        ...
        child: AlertDialog(
          ...
          content: const Text(
            'Use your device fingerprint or lock to unlock apps faster.',
  ...
  void _verify(String code) {
    final method =
        _type == AppLockSecurityRepository.typePin ? 'pin' : 'pattern';
    AppAnalytics.secureAuthAttempt(
        miniApp: AppAnalytics.miniAppAppLock, method: method);
    if (_sec.verify(code)) {
      AppAnalytics.secureAuthResult(
          miniApp: AppAnalytics.miniAppAppLock, method: method, success: true);
      _sec.markUnlocked();
      widget.onUnlocked();
      return;
    }
    AppAnalytics.secureAuthResult(
        miniApp: AppAnalytics.miniAppAppLock, method: method, success: false);
    _fail(_type == AppLockSecurityRepository.typePin
        ? 'Wrong PIN'
        : 'Wrong pattern');
  }
  ...
  Widget _typeChooser() {
    ...
    return Container(
      ...
      child: Row(
        children: [
          seg('PIN', AppLockSecurityRepository.typePin),
          seg('Pattern', AppLockSecurityRepository.typePattern),
        ],
      ),
    );
  }
...
/// The little "1 — 2" progress chips shown on the confirm step.
