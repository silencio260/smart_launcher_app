...
/// First-run setup and subsequent unlock for App Hider. Monochrome, modelled on
/// the Vault's lock screen (custom PIN pad + pattern + fingerprint shortcut) so
/// the three privacy mini-apps feel identical.
///
/// Calls [onUnlocked] once the user is in; [onCancel] when they back out.
class AppHiderLockScreen extends StatefulWidget {
  final AppHiderSecurityRepository security;
  ...
  @override
  State<AppHiderLockScreen> createState() => _AppHiderLockScreenState();
}

class _AppHiderLockScreenState extends State<AppHiderLockScreen> {
  ...
  AppHiderSecurityRepository get _sec => widget.security;
  ...
  void _handleSetupCode(String code) {
    ...
    if (code != _first) {
      setState(() {
        _step = 0;
        _first = null;
        _error = _type == AppHiderSecurityRepository.typePin
    ...
    final result = await showDialog<bool>(
      ...
      builder: (context) => Theme(
        ...
        child: AlertDialog(
          ...
          content: const Text(
            'Use your device fingerprint or lock to open App Hider faster.',
  ...
  Widget _typeChooser() {
    ...
    return Container(
      ...
      child: Row(
        children: [
          seg('PIN', AppHiderSecurityRepository.typePin),
          seg('Pattern', AppHiderSecurityRepository.typePattern),
        ],
      ),
    );
  }
