// Vendored from Flutter 3.35.1 (packages/flutter/lib/src/widgets/reorderable_list.dart).
// Patched so neighbor swap requires the dragged proxy to cross 60% of an item
// (vs. Flutter's stock 50%). Auto-scroll behavior is unchanged. All public
// symbols are prefixed `Threshold` to avoid clashes with Flutter's originals.
// Original copyright:
...
/// A callback used by [ThresholdReorderableList] to report that a list item has moved
...
/// See also:
///
///  * [ThresholdReorderableList], a widget list that allows the user to reorder
///    its items.
///  * [SliverThresholdReorderableList], a sliver list that allows the user to reorder
///    its items.
///  * [ReorderableListView], a Material Design list that allows the user to
///    reorder its items.
typedef ThresholdReorderCallback = void Function(int oldIndex, int newIndex);

/// Signature for the builder callback used to decorate the dragging item in
/// [ThresholdReorderableList] and [SliverThresholdReorderableList].
///
...
typedef ThresholdReorderItemProxyDecorator =
...
///    slivers: <Widget>[
///      SliverThresholdReorderableList(
///        itemBuilder: (BuildContext context, int index) {
///          return ThresholdReorderableDragStartListener(
...
typedef ThresholdReorderDragBoundaryProvider = DragBoundaryDelegate<Rect>? Function(BuildContext context);
...
/// [ThresholdReorderableListState.startItemDragReorder]. This is most easily achieved
/// by wrapping each child in a [ThresholdReorderableDragStartListener] or a
/// [ThresholdReorderableDelayedDragStartListener]. These will take care of recognizing
/// the start of a drag gesture and call the list state's
/// [ThresholdReorderableListState.startItemDragReorder] method.
///
/// This widget's [ThresholdReorderableListState] can be used to manually start an item
/// reorder, or cancel a current drag. To refer to the
/// [ThresholdReorderableListState] either provide a [GlobalKey] or use the static
/// [ThresholdReorderableList.of] method from an item's build method.
///
/// See also:
///
///  * [SliverThresholdReorderableList], a sliver list that allows the user to reorder
...
///  * [ReorderableListView], a Material Design list that allows the user to
///    reorder its items.
class ThresholdReorderableList extends StatefulWidget {
  ...
  const ThresholdReorderableList({
    super.key,
  ...
  /// (usually a [ThresholdReorderableDragStartListener] or
  /// [ThresholdReorderableDelayedDragStartListener]).
  /// {@endtemplate}
  ...
  /// of the items.
  /// {@endtemplate}
  final ThresholdReorderCallback onReorder;
  ...
  /// {@endtemplate}
  final ThresholdReorderItemProxyDecorator? proxyDecorator;
  ...
  ///
  /// {@macro flutter.widgets.SliverThresholdReorderableList.autoScrollerVelocityScalar.default}
  ...
  /// Defaults to null.
  /// {@endtemplate}
  final ThresholdReorderDragBoundaryProvider? dragBoundaryProvider;

  /// The state from the closest instance of this class that encloses the given
  /// context.
  ///
  /// This method is typically used by [ThresholdReorderableList] item widgets that
  /// insert or remove items in response to user input.
  ///
  /// If no [ThresholdReorderableList] surrounds the given context, then this function
  ...
  ///    [ThresholdReorderableList] ancestor is found.
  static ThresholdReorderableListState of(BuildContext context) {
    final ThresholdReorderableListState? result = context.findAncestorStateOfType<ThresholdReorderableListState>();
    assert(() {
      if (result == null) {
        throw FlutterError.fromParts(<DiagnosticsNode>[
          ErrorSummary(
            'ThresholdReorderableList.of() called with a context that does not contain a ThresholdReorderableList.',
          ),
          ErrorDescription(
            'No ThresholdReorderableList ancestor could be found starting from the context that was passed to ThresholdReorderableList.of().',
          ),
          ErrorHint(
            'This can happen when the context provided is from the same StatefulWidget that '
            'built the ThresholdReorderableList. Please see the ThresholdReorderableList documentation for examples '
            'of how to refer to an ThresholdReorderableListState object:\n'
            '  https://api.flutter.dev/flutter/widgets/ThresholdReorderableListState-class.html',
          ),
  ...
  /// context.
  ///
  /// This method is typically used by [ThresholdReorderableList] item widgets that insert
  /// or remove items in response to user input.
  ///
  /// If no [ThresholdReorderableList] surrounds the context given, then this function will
  /// return null.
  ///
  /// This method can be expensive (it walks the element tree).
  ///
  /// See also:
  ///
  ///  * [of], a similar function that will throw if no [ThresholdReorderableList] ancestor
  ///    is found.
  static ThresholdReorderableListState? maybeOf(BuildContext context) {
    return context.findAncestorStateOfType<ThresholdReorderableListState>();
  }

  @override
  ThresholdReorderableListState createState() => ThresholdReorderableListState();
}
...
/// can refer to the [ThresholdReorderableList]'s state with a global key:
///
/// ```dart
/// GlobalKey<ThresholdReorderableListState> listKey = GlobalKey<ThresholdReorderableListState>();
/// // ...
...
/// ```
class ThresholdReorderableListState extends State<ThresholdReorderableList> {
  final GlobalKey<SliverThresholdReorderableListState> _sliverReorderableListKey = GlobalKey();
  ...
  /// [ThresholdReorderableDragStartListener] or [ThresholdReorderableDelayedDragStartListener]
    ...
    return CustomScrollView(
      ...
      slivers: <Widget>[
        SliverPadding(
          ...
          sliver: SliverThresholdReorderableList(
...
/// [SliverThresholdReorderableListState.startItemDragReorder]. This is most easily
/// achieved by wrapping each child in a [ThresholdReorderableDragStartListener] or
/// a [ThresholdReorderableDelayedDragStartListener]. These will take care of
/// recognizing the start of a drag gesture and call the list state's start
/// item drag method.
///
/// This widget's [SliverThresholdReorderableListState] can be used to manually start an item
/// reorder, or cancel a current drag that's already underway. To refer to the
/// [SliverThresholdReorderableListState] either provide a [GlobalKey] or use the static
/// [SliverThresholdReorderableList.of] method from an item's build method.
///
/// See also:
///
///  * [ThresholdReorderableList], a regular widget list that allows the user to reorder
///    its items.
///  * [ReorderableListView], a Material Design list that allows the user to
///    reorder its items.
class SliverThresholdReorderableList extends StatefulWidget {
  /// Creates a sliver list that allows the user to interactively reorder its
  /// items.
  ///
  /// The [itemCount] must be greater than or equal to zero.
  const SliverThresholdReorderableList({
    super.key,
    required this.itemBuilder,
  ...
  /// {@macro flutter.widgets.EdgeDraggingAutoScroller.velocityScalar}
  ///
  /// {@template flutter.widgets.SliverThresholdReorderableList.autoScrollerVelocityScalar.default}
  ...
  @override
  SliverThresholdReorderableListState createState() => SliverThresholdReorderableListState();

  /// The state from the closest instance of this class that encloses the given
  /// context.
  ///
  /// This method is typically used by [SliverThresholdReorderableList] item widgets to
  /// start or cancel an item drag operation.
  ///
  /// If no [SliverThresholdReorderableList] surrounds the context given, this function
  /// will assert in debug mode and throw an exception in release mode.
  ///
  /// This method can be expensive (it walks the element tree).
  ///
  ...
  ///
  ///  * [maybeOf], a similar function that will return null if no
  ///    [SliverThresholdReorderableList] ancestor is found.
  static SliverThresholdReorderableListState of(BuildContext context) {
    final SliverThresholdReorderableListState? result = context
        .findAncestorStateOfType<SliverThresholdReorderableListState>();
    assert(() {
      if (result == null) {
        throw FlutterError.fromParts(<DiagnosticsNode>[
          ErrorSummary(
            'SliverThresholdReorderableList.of() called with a context that does not contain a SliverThresholdReorderableList.',
          ),
          ErrorDescription(
            'No SliverThresholdReorderableList ancestor could be found starting from the context that was passed to SliverThresholdReorderableList.of().',
          ),
          ErrorHint(
            'This can happen when the context provided is from the same StatefulWidget that '
            'built the SliverThresholdReorderableList. Please see the SliverThresholdReorderableList documentation for examples '
            'of how to refer to an SliverThresholdReorderableList object:\n'
            '  https://api.flutter.dev/flutter/widgets/SliverThresholdReorderableListState-class.html',
          ),
          context.describeElement('The context used was'),
        ]);
      }
  ...
  /// context.
  ///
  /// This method is typically used by [SliverThresholdReorderableList] item widgets that
  /// insert or remove items in response to user input.
  ///
  /// If no [SliverThresholdReorderableList] surrounds the context given, this function
  /// will return null.
  ...
  /// See also:
  ///
  ///  * [of], a similar function that will throw if no [SliverThresholdReorderableList]
  ///    ancestor is found.
  static SliverThresholdReorderableListState? maybeOf(BuildContext context) {
    return context.findAncestorStateOfType<SliverThresholdReorderableListState>();
  }
}
...
///
/// An app that needs to start a new item drag or cancel an existing one
/// can refer to the [SliverThresholdReorderableList]'s state with a global key:
///
/// ```dart
/// // (e.g. in a stateful widget)
/// GlobalKey<SliverThresholdReorderableListState> listKey = GlobalKey<SliverThresholdReorderableListState>();
///
/// // ...
///
/// @override
/// Widget build(BuildContext context) {
///   return SliverThresholdReorderableList(
///     key: listKey,
///     itemBuilder: (BuildContext context, int index) => const SizedBox(height: 10.0),
///     itemCount: 5,
///     onReorder: (int oldIndex, int newIndex) {
...
/// ```
///
/// [ThresholdReorderableDragStartListener] and [ThresholdReorderableDelayedDragStartListener]
/// refer to their [SliverThresholdReorderableList] with the static
/// [SliverThresholdReorderableList.of] method.
class SliverThresholdReorderableListState extends State<SliverThresholdReorderableList>
  ...
  @protected
  @override
  void didUpdateWidget(covariant SliverThresholdReorderableList oldWidget) {
  ...
  void _dragUpdateItems() {
    ...
    for (final _ReorderableItemState item in _items.values) {
      ...
      // Swap threshold: require the proxy to overlap a neighbor by >60%
      // (vs. Flutter's stock 50% midpoint). `forwardThreshold` is used when
      // the proxy's leading edge approaches from below; `backwardThreshold`
      // when its trailing edge approaches from above.
      const double swapThresholdFraction = 0.6;
      final double itemForwardThreshold =
          itemStart + itemExtent * swapThresholdFraction;
      final double itemBackwardThreshold =
          itemStart + itemExtent * (1.0 - swapThresholdFraction);

      if (_reverse) {
        if (itemEnd >= proxyItemEnd && proxyItemEnd >= itemForwardThreshold) {
          ...
          newIndex = item.index;
          break;
        } else if (itemBackwardThreshold >= proxyItemStart && proxyItemStart >= itemStart) {
        ...
        if (item.index == _dragIndex!) {
          ...
          if (itemForwardThreshold <= proxyItemEnd && proxyItemEnd <= itemEnd) {
            newIndex = _dragIndex!;
          }
        } else if (itemStart <= proxyItemStart && proxyItemStart <= itemBackwardThreshold) {
          // The start of the proxy is in the beginning half of the item, so
          // we should swap the item with the gap and we are done looking for
          // the new index.
          newIndex = item.index;
          break;
        } else if (itemForwardThreshold <= proxyItemEnd && proxyItemEnd <= itemEnd) {
          // The end of the proxy is in the ending half of the item, so
          // we should swap the item with the gap and we are done looking for
          // the new index.
          newIndex = item.index + 1;
...
class _ReorderableItemState extends State<_ReorderableItem> {
  late SliverThresholdReorderableListState _listState;
  ...
  @override
  void initState() {
    _listState = SliverThresholdReorderableList.of(context);
...
/// See also:
///
///  * [ThresholdReorderableDelayedDragStartListener], a similar wrapper that will
...
///  * [ReorderableListView], a Material Design list that allows the user to
///    reorder its items.
class ThresholdReorderableDragStartListener extends StatelessWidget {
  ...
  /// handle.
  const ThresholdReorderableDragStartListener({
    super.key,
    required this.child,
    required this.index,
    this.enabled = true,
  ...
  void _startDragging(BuildContext context, PointerDownEvent event) {
    ...
    final SliverThresholdReorderableListState? list = SliverThresholdReorderableList.maybeOf(context);
...
/// See also:
///
///  * [ThresholdReorderableDragStartListener], a similar wrapper that will
...
///  * [ReorderableListView], a Material Design list that allows the user to
///    reorder its items.
class ThresholdReorderableDelayedDragStartListener extends ThresholdReorderableDragStartListener {
  ...
  /// list.
  const ThresholdReorderableDelayedDragStartListener({
    super.key,
...
class _DragInfo extends Drag {
  ...
  late SliverThresholdReorderableListState listState;
  late int index;
  late Widget child;
...
class _DragItemProxy extends StatelessWidget {
  ...
  final SliverThresholdReorderableListState listState;
  final int index;
  final Widget child;
...
class _ReorderableItemGlobalKey extends GlobalObjectKey {
  ...
  final Key subKey;
  final int index;
  final SliverThresholdReorderableListState state;
